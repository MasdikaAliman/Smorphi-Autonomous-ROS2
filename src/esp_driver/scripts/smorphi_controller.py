...
class SmorphiController(Node):
    def __init__(self):
        ...
        self.create_subscription(String, "shape_need", self.shpneed, 10)
    ...
    def writespeed(self, msg):
        ...
        cmd_vel = str(cm_Vx)+","+str(cm_Vy)+"," + \
            str(cm_Wz)+","+str(cm_shape)+"\n"
        # print(cmd_vel.encode('utf-8'))
        self.ser.write(cmd_vel.encode('utf-8'))
        self.ser.write(b'\n')
    ...
    def encoder_read(self):
        ...
        if len(encoder_readings) > 5:
            ...
            # print(len(encoder_readings))

            if (len(encoder_readings) == 4):
        ...
        # self.y += delta_y
        # self.th += self.w
        # [quat_x, quat_y, quat_z, quat_w] = tf_transformations.quaternion_from_euler(
        #     0, 0, self.th)
        # odom = Odometry()
